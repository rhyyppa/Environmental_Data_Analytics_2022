...
> ANSWER:
> The EPSG code is 4269. This is a projected coordinate referene system. 
...
> ANSWER:
> dec_long_va contains the x values and dec_lat_va contains the y values.
...
>ANSWER: 
> "geometry" was added as a new field. Both "dec_long_va" and "dec_lat_va" dissapeared from the dataframe. 
...
ggplot() +
  ...
  geom_sf(data=gage.sf, color='red', shape=1) +
...
height_data <- read.csv("../Data/Raw/NWIS_SiteFlowData_NE_RAW.csv", 
                        colClasses = c('site_no' = 'factor'))
...
site_sf_join <-  merge(x = gage.sf,
                           y = height_data, 
                           by.x = "site_no", 
                           by.y = "site_no")

#17. Show the column names of the joined dataset
colnames(site_sf_join)
...
mapview(counties_sf,
  ...
  mapview(site_sf_join, cex = "gage_ht")
...
gage_sf_utm <- st_transform(site_sf_join, 32614)

#21 Plot the data
mapview(counties_sf_utm,
        col.regions = brewer.pal(2, 'RdBu')) + 
  mapview(gage_sf_utm, cex = "gage_ht")
...
lancaster_gages <- gage_sf_utm[lancaster_sf,]

#24 Plot
ggplot() +
  geom_sf(data = counties_sf_utm, color = "black") +
  geom_sf(data = lancaster_sf, color = "yellow") +
