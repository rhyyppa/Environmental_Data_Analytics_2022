...
Coefficiemnts:
- depth has a negative coefficient so that two variables are negatively related. In other words, as depth increases, the irradiance level decreases. 

 p-level is less than 0.05 (alpha) therefore, our coefficients are statistically different than 0.
 r^2 --> how much of the variability in the response variable is explained by the explanatory variable
 
 Standard error --> want this value to be as small as possible
...
```{r, fig.height = 3, fig.width = 4}
# step 1 filter data removing any zero values and outliers (above 5000)
