...
Seasonal Data -- A time series variable exhibiting a repeating pattern or regular intervals over time

Autocorrelation Function
Correleation: is the joint variability of two variables

Autocorrelation is a measure of dependence between two adjacent values of the same variable.
-> it gives information about the dependence strucuture of a time series


Trend Component
1. Long-term tendency 
-> Increase (upward movement), or
-> Decrease (downward movement)

2. Trend can be linear or non-linear 

Stochastic vs. deterministic trends

Deterministic trends can be either linear or polynomial
Detrending is accompanied by running a regression and obtaining the series of residuals. The residuals will give you the detrended series.
This is call trend-stationarity

A stochastic trend is a trend that changes overtime, in other words, is a series that is called difference-stationarity.
Detrending a stochastic trend requires differencing the series.
...
-> cannot be applied to stationary data
-> Ho = stationary & Ha = follows a trend
-> the test will check the magnitude of S and its sginificance based on the number of observations
-> In other words, the bigger the # of observations, the higher S will need to be statistically significant

2. Spearman's Rank Correlation Coefficient
-> a statistical measure of the strength of a monotonic relationship
-> the relationship does NOT need to be linear
-> so if one variable increases and so does the other it does not matter the proportion of the increase
-> if the correlation is close to 0, then there is no trend
-> cor() or cor.test() from package "stats"

3. Dick-Fuller Test
-> To verify if there is a stochastic trend in the data or unit root in the time series
-> Ho = contains a unit root, Ha = stationary 
-> for a more general test (more legs), you can use the Augmented Dickey-Fuller (ADF) test, using adf.test() in package tseries.
